import { normalizeCircuitSpec } from "./circuit-diagram.js";
import { normalizeDiagramType } from "./study-diagram-meta.js";

export const SPEC_DIAGRAM_TYPES = new Set(["recursion_tree", "number_line", "circuit", "circuit_diagram"]);
...
export function normalizeDiagramSpec(raw) {
  ...
  const rawType = String(raw.diagramType || raw.type || "")
    .trim()
    .toLowerCase()
    .replace(/[\s-]+/g, "_");
  if (rawType === "circuit" || rawType === "circuit_diagram") {
    return normalizeCircuitSpec(raw);
  }
