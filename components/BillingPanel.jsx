...
function planSwitchCopy(billing, interval, canChangePlan, yearlyDiscountPercent) {
  ...
  if (billing.pendingCheckout) {
    return interval === "month"
      ? `Card required — ${billing.trialDaysLeft || 7}-day free trial, then monthly billing.`
      : `One payment per year — about ${yearlyDiscountPercent}% less than paying monthly (set your yearly price in Stripe).`;
  }
...
export default function BillingPanel({ billing, subscription, stripeEnabled, yearlyDiscountPercent }) {
  ...
  return (
    <section className="panel">
      ...
      <AutoRenewControl apiPath="/api/billing/auto-renew" billing={billing} canManage={canChangePlan} />

      <UsageMeters subscription={subscription} />

      {!stripeEnabled ? (
        <div className="message warning">
          Stripe is not configured. Add API keys and price IDs in <code>.env.local</code> to collect payments.
        </div>
      ) : (
