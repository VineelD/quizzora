...
import { isOpenAiDisabled, resolveQuizProvider } from "./openai-policy.js";
import { generateQuizFromQuestionBank } from "./quiz-from-question-bank.js";
import {
  buildQuizDiagramRulesBlock,
  buildQuizQuestionJsonSchema,
  buildQuizRenderabilityReviewRules,
  buildQuizReviewQuestionsJsonSchema,
  buildQuizSingleQuestionJsonSchema,
...
export function buildOpenAiPrompt(
  ...
  const diagramRules = buildQuizDiagramRulesBlock({ includeDiagramMarking });
...
${buildQuizQuestionJsonSchema(yearLevel)}
...
export async function generateQuiz(request, { onProgress } = {}) {
  if (resolveQuizProvider() === "question_bank") {
    return generateQuizFromQuestionBank(request, { onProgress });
  }
...
function buildQuizSelfReviewPrompt(questions, request) {
  const compact = questions.map((question, index) => ({
    ...
    diagramCaption: question.diagramCaption,
...
Do not change correct answers. You may fix diagram routing, labels, and values when improving renderability.
...
async function reviewQuizQuestionsWithLlm(questions, request) {
  ...
  if (process.env.QUIZ_LLM_SELF_REVIEW === "false" || isOpenAiDisabled() || resolveQuizProvider() === "question_bank") {
    return questions;
  }
  ...
  return questions.map((original, index) => {
    ...
    return {
      ...
      diagramType: patch.diagramType ?? original.diagramType,
      ...
      diagramTitle: patch.diagramTitle ?? original.diagramTitle,
      diagramCaption: patch.diagramCaption ?? original.diagramCaption,
      diagramLabels: patch.diagramLabels ?? original.diagramLabels,
    };
  });
}
...
export function buildSingleQuestionRegenerationPrompt(request, { index, issues = [] } = {}) {
...
${buildQuizDiagramRulesBlock({ includeDiagramMarking: true })}
`.trim();
}
...
export async function ensureQuizQuestionsClarity(questions, request) {
  ...
  if (isOpenAiDisabled() || resolveQuizProvider() === "question_bank") {
    return questions.map((question) =>
      normalizeQuizQuestionFields(applyQuizQuestionClarityFixes(question)),
    );
  }
