...
import { ensureQuestionBankSchema } from "./db-migrate-question-bank.js";
import { ensureCurriculumVectorsSchema } from "./db-migrate-curriculum-vectors.js";
import { ensureQuestionEmbeddingsSchema } from "./db-migrate-question-embeddings.js";
...
function getDbPath() {
  return process.env.SQLITE_DATABASE_PATH || join(process.cwd(), "data", "littlecode.sqlite");
}
...
function migrate(db) {
  ...
  ensureQuestionEmbeddingsSchema(db);
  ensureCurriculumVectorsSchema(db);
  purgeDemoUsers(db);
  ensureProfiles(db);
}
