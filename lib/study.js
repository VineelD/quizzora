...
import { generateStudyCoachReply, generateStudyCoachReplyStream } from "./study-coach.js";
...
export async function postStudyMessage({ studentId, assignmentId, message, requestNarration = false }) {
  ...
  if (
    coach.responseId &&
    (coach.source === "openai" || coach.source === "hybrid_openai" || coach.source === "mock")
  ) {
...
export async function postStudyMessageStream({
  studentId,
  assignmentId,
  message,
  ...
  const coach = await generateStudyCoachReplyStream({
    context,
    history,
    message: trimmed,
    previousResponseId,
  ...
  logAudit({
    ...
    summary: coach.flagged
      ...
      : studentOnTopic
        ? "Student used Study Coach on topic"
        : "Student sent an off-topic Study Coach message",
    metadata: { source: coach.source, flagged: coach.flagged, onTopic: studentOnTopic, streamed: coach.streamed },
  });
