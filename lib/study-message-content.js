...
function countDictionaryWordHits(token) {
  const lower = String(token || "").toLowerCase();
  const glueWords = GLUED_WORD_BOUNDARIES.filter((word) => word.length >= 2).sort(
  ...
  while (cursor < lower.length) {
    ...
    for (const word of glueWords) {
      if (lower.startsWith(word, cursor)) {
        hits += 1;
        cursor += word.length;
        matched = true;
        break;
...
function segmentNeedsGluedWordSplit(segment) {
  ...
  if (letters > 0 && spaces >= 2 && spaces / letters >= 0.07) {
    return false;
  }

  if (/[A-Za-z]{14,}/.test(value)) {
    return true;
  }
  if (spaces === 0 && value.length >= 10 && countDictionaryWordHits(value) >= 2) {
    return true;
  }
  if (
    /[a-z]{4,}(?:with|and|the|for|has|are|per|rate|given|population|growth|formula|annual|monthly)[a-z]{4,}/i.test(
      value,
    )
  ) {
    return true;
  }
  if (spaces === 0 && letters >= 18) {
    return true;
  }
  return spaces <= 1 && value.length >= 28 && /[a-z]{4,}[a-z]{4,}/i.test(value);
}

function isLikelySingleEnglishWord(run) {
  const word = String(run || "").trim();
  if (word.length < 8 || word.length > 22 || /\s/.test(word)) {
    return false;
  }
  ...
  return GLUED_WORD_BOUNDARIES.some(
    (entry) => entry.length >= 4 && entry.toLowerCase() === word.toLowerCase(),
  );
}

function splitGluedEnglishWords(segment) {
  let value = String(segment || "");
  if (!value || !segmentNeedsGluedWordSplit(value)) {
    return value;
  }
  ...
  while (index < value.length) {
    ...
    if (run.length < 8 || /\s/.test(run) || isLikelySingleEnglishWord(run)) {
      output += run;
      continue;
    }
...
export function fixGluedWordProblemSpacing(text) {
  ...
  const transformSegment = (segment) => {
    ...
    outside = outside.replace(/(?<=[a-z]{2,})(?=[A-Z][a-z])/g, " ");
    return outside
      .split(/(\s+)/)
      .map((token) => (token.trim() ? splitGluedEnglishWords(token) : token))
      .join("");
  };
...
const ASCII_ANY_FENCE_PATTERN = /```[^\n`]*\n[\s\S]*?```/g;

const ASCII_ARROW_FLOW_LINE = /^\s*.+\s*-->\s*.+$/m;

const ASCII_CIRCUIT_LINE =
  /^\s*\+?\s*---+.*(?:Ω|ohm|resistor|R\d|V\s+supply)/i;

const ASCII_DIAGRAM_HEADER =
  /^#{1,3}\s*(?:.*\b(?:Visual|Diagram|Circuit)\b.*|(?:Diagram|Labels|Flow|Tree|Structure|Visual):)[^\n]*$/i;

const ASCII_VISUAL_TITLE_LINE =
  /^(?:\*{1,2}).*\b(?:Circuit\s+)?Visual\b.*(?:\*{1,2})?$/i;

const ASCII_COLOR_EXPLANATION_HEADER =
  /^#{0,3}\s*\*{0,2}Explanation of Colors:?\*{0,2}\s*$/i;

const ASCII_VISUAL_UPSELL_LINE =
  /\bif you want,?\s+i can (?:create|make|generate)\b/i;

/** True when text looks like an ASCII / text-art diagram (circuits, flows, boxes). */
export function isAsciiDiagramContent(text) {
  const value = String(text || "").trim();
  if (!value) {
    return false;
  }
  if (/```\s*mermaid/i.test(value)) {
    return false;
  }
  if (ASCII_DIAGRAM_BODY_PATTERN.test(value)) {
    return true;
  }
  if (/R\d\s*\([^)]*(?:Ω|ohm)/i.test(value)) {
    return true;
  }
  if (/\+\s*---+.*(?:Ω|ohm)/i.test(value)) {
    return true;
  }
  if (/\d+\s*V\s+supply/i.test(value) && /---/.test(value)) {
    return true;
  }
  if (/\[R\d\][^\n]*\[R\d\]/i.test(value)) {
    return true;
  ...
  return lines.some((line) => isAsciiDiagramLine(line));
}
...
function isAsciiDiagramLine(line) {
  ...
  if (ASCII_VISUAL_TITLE_LINE.test(trimmed)) {
    return true;
  }
  if (ASCII_ARROW_FLOW_LINE.test(trimmed)) {
    return true;
  }
  if (ASCII_CIRCUIT_LINE.test(trimmed)) {
    return true;
  }
  if (/^R\d\s*\([^)]*(?:Ω|ohm)/i.test(trimmed)) {
    return true;
  }
  if (/^\[R\d\]/i.test(trimmed) || /\[Battery/i.test(trimmed) || /\[GND\]/i.test(trimmed)) {
    return true;
  }
  if (/^\+-{4,}/.test(trimmed)) {
    return true;
  }
  if (/\|\s*I(?:[₁₂]|[0-9])?\s*=/.test(trimmed)) {
    return true;
  }
  if (ASCII_COLOR_EXPLANATION_HEADER.test(trimmed)) {
    return true;
  }
  if (/^[-*]\s+\*{0,2}(?:Red|Blue|Green)\s+(?:resistor|battery)/i.test(trimmed)) {
    return true;
  }
  if (ASCII_VISUAL_UPSELL_LINE.test(trimmed)) {
    return true;
  }
  if (/^[-*]\s+Currents?\s+I(?:[₁₂]|[0-9])/i.test(trimmed)) {
    return true;
  }
  if (/^\|\s*\|/.test(trimmed)) {
    return true;
...
function stripAsciiCodeFences(text) {
  return String(text || "").replace(ASCII_ANY_FENCE_PATTERN, (block) => {
    if (/```\s*mermaid/i.test(block)) {
      return block;
    }
    const inner = block.replace(/^```[^\n]*\n?/, "").replace(/\n?```\s*$/, "");
    return isAsciiDiagramContent(inner) ? "" : block;
  });
}
...
export function stripAsciiDiagramArtifacts(text) {
  ...
  while (index < lines.length) {
    ...
    if (ASCII_VISUAL_TITLE_LINE.test(line.trim()) || ASCII_COLOR_EXPLANATION_HEADER.test(line.trim())) {
      index += 1;
      while (index < lines.length) {
        ...
        if (
          isAsciiDiagramLine(next) ||
          /^[-*]\s/.test(next) ||
          ASCII_VISUAL_UPSELL_LINE.test(next)
        ) {
          index += 1;
          continue;
        }
  ...
  value = stripAsciiCodeFences(value);
  value = value
    .split("\n")
    .filter((line) => !/^```(?:text)?\s*$/i.test(line.trim()))
    .join("\n");
