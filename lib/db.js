...
export function getDefaultSchoolId() {
  const school = getDb().prepare("SELECT id FROM schools ORDER BY id LIMIT 1").get();
  if (!school) {
    throw new Error("No schools exist. Create or register a school before requesting a default school.");
  }
  return Number(school.id);
}

export function ensureTestSchool({ name = "Test School" } = {}) {
  const db = getDb();
  const existing = db.prepare("SELECT * FROM schools ORDER BY id LIMIT 1").get();
  if (existing) {
    return toPlainRow(existing);
  }
  const result = db.prepare("INSERT INTO schools (name) VALUES (?)").run(name);
  return getSchoolById(Number(result.lastInsertRowid));
}
...
export function createTestTeacher({ name, email, username, password }) {
  const db = getDb();
  const passwordHash = hashPassword(password);
  const schoolId = ensureTestSchool().id;
  const result = db
    .prepare(
      `
      INSERT INTO users (name, username, email, role, password_hash, email_verified_at, school_id)
