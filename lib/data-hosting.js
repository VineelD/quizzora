...
export function getDataHostingParagraphs() {
  ...
  return [
    `Student, school, family, and account data is processed and stored on servers located at the operator's premises in ${DATA_HOSTING_REGION} (on-premises hosting). The primary application database is SQLite on a dedicated Windows server at those premises.`,
    `We do not sell personal information to third parties for marketing purposes.`,
    `We use third-party processors only where necessary to operate ${product}: OpenAI for AI-powered quiz generation, study tools, and related features (prompts and content needed for those features may be sent to OpenAI for processing); email delivery providers (for example Resend or configured SMTP) for transactional messages such as password resets and invitations; and optional external payment providers only if you choose to make a voluntary support contribution (we do not store raw card numbers).`,
...
export function dataHostingSignupHint() {
  return `Data is stored on ${OPERATOR_PRODUCT_NAME} servers at the operator's premises in ${DATA_HOSTING_REGION}. We take reasonable steps to safeguard your data and privacy.`;
}
