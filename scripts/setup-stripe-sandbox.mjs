...
if (!testSecret) {
  ...
  console.error("3. Run:");
  console.error("   $env:STRIPE_TEST_SECRET_KEY='sk_test_...'; npm run stripe:sandbox");
  console.error("");
  console.error("Or add STRIPE_TEST_SECRET_KEY=sk_test_... to .env.local and run again.");
  process.exit(1);
}
...
const lines = [
  ...
  webhook.secret ? `STRIPE_WEBHOOK_SECRET=${webhook.secret}` : "# STRIPE_WEBHOOK_SECRET=whsec_...  # from Stripe Dashboard (test webhook)",
